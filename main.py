...
station = found_stations_by_name[
    0]  # first entry in array is the station name. Used to check if given station name is valid.
...
trains_with_changes = timetable_helper.get_timetable_changes(
    trains_in_this_hour)  # getting all trains with changes at given time
    ...
    if content_type == 'text' and str(chat_id) == telegramData.telegram_chat_id:
        ...
        if msg['text']:  # handle Bahnhof command
            ...
            if message.startswith('Bahnhof'):  # set station
                ...
                if updateBahnhof(Bahnhof,
                                 Uhrzeit) == 1:  # if entered trainstation is valid, trainstation will get updated
        ...
        if msg['text']:  # handle watch command
            ...
            if message.startswith(
                    'watch'):  # user input is divided with ":". Filtering the given information for futher processing
                ...
                if hour.isnumeric() and min.isnumeric() == 1:  # error handling: only numbers for time input allowed
                    ...
                    try:
                        ...
                        if 0 <= hour <= 24 and 0 <= min <= 60:  # only specific numbers allowed
                            ...
                            trains_with_changes = timetable_helper.get_timetable_changes(
                                trains_at_given_hour)  # get trains with changes
                            flag = 0  # flag to check if train actually exists

                            for i, train in enumerate(
                                    trains_with_changes):  # check all trains in given hour to find if the one at given hour+minute exists

                                if str(hour) + str(
                                        min) in train.departure and zielBahnhof in train.stations:  # check if train from user inout exists
